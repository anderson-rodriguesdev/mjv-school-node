...
router.get("/:id", (req: Request, res: Response) => {
  const findProduct = productsList.products.find(
...
router.post("/", (req: Request, res: Response) => {
  if (req.body) {
    productsList.products.push(req.body);
...
router.put("/:id", (req: Request, res: Response) => {
  const findProduct = productsList.products.findIndex(
    (item) => item.id === Number(req.params.id)
  );
  ...
  productsList.products[findProduct] = req.body;
...
router.delete("/delete/:id", (req: Request, res: Response) => {
  ...
  productsList.products.splice(findProduct, 1);
